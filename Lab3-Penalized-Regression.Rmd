...
# Exercise: performing penalized logistic regression with `glmnet`

## Tasks {-}

#### 1. Use the same `grid` of $\gamma$s from before and fit a ridge logistic regression model with them with the binary outcome variable `Ybin` and predictors `X`. {-}

*Hint:* in the `glmnet()` function use `family = "binomial"` for logistic regression.

<details><summary>Solution</summary>
```{r}
# Binary version
grid <- seq(1, 1000, by = 10)  # same grid as before

bin_ridge_model <- glmnet(X, Ybin, alpha = 0, lambda = grid, family = "binomial")
```
</details>

#### 2. Fit a lasso logistic model with the binary outcome variable `Ybin` and predictors `X`. {-}

Again you should use the binomial family.

<details><summary>Solution</summary>
```{r}
bin_lasso_model <- glmnet(X, Ybin, alpha = 1, family = "binomial")
```
</details>
...
<details><summary>Solution</summary>
```{r, fig.show='hold'}
plot(bin_ridge_model, xvar = "lambda", xlab = "log(gamma)")
title("Ridge logistic regression", line = -1)

plot(bin_lasso_model, xvar = "lambda", xlab = "log(gamma)")
title("Lasso logistic regression", line = -1)
```
</details>

Again we can ask the question which $\gamma$ is the right one? Generally the
number of PCs in PC regression and $\gamma$ in the ridge and Lasso
regression are called __tuning parameters__. The different number of
PCs/$\gamma$ correspond to different models which can be represented
as a class of models $\mathcal{M}$. We now look at how to choose turning
parameters and hence optimal prediction models from $\mathcal{M}$.


# Evaluation of prediction models and tunign hyperparameters
...
Different performance metrics will be used:

-   For continuous outcomes we will use the mean squared error (MSE).

-   For a binary outcome (as in the logistic regression models), 
    we will use 
    
    * the area under the receiver operating characteristic curve
    ([AUROC or AUC](https://en.wikipedia.org/wiki/Receiver_operating_characteristic)) 
    
    * the misclassification error based on a
    [confusion matrix](https://en.wikipedia.org/wiki/Confusion_matrix),
    i.e. the proportion of misclassified observations.


We use the test subset for final model evaluation. No estimation of
model parameters whatsoever is done on the test dataset.

Let’s begins by creating our cost/evaluation measure functions. 
For the AUC function, we simply use the function `auc` from the 
*[pROC](https://CRAN.R-project.org/package=pROC)* package.

```{r performance-metric-functions}
# Mean Squared Error
...
MISERR <- function(obs, pred, cutoff = 0.5) {
  # Check if we have same number of observations and predictions
  stopifnot(length(obs) == length(pred))
  
  n <- length(obs)
  
  # Turn predictions into binary response
  ypred <- as.numeric(pred > cutoff)
  
  # Create confusion matrix
  tab <- table(obs, ypred)
  
  # Correct observations are on the diagonal
  # Error = 1 - the proportion of correct predictions
  1 - sum(diag(tab)) / n
}
```

To see how the misclassification error works and what a confusion matrix looks
like, imagine we have a set of 10 observations and 10 corresponding predictions
from a logistic regression model (these will be numbers between 0 and 1).
We use a cutoff of 0.5 to classify a prediction as either 1 or 0.

```{r}
set.seed(123)
n <- 10
(obs <- sample(c(0, 1), n, replace = TRUE))  # 10 binary observations
(pred <- runif(n))  # 10 predictions, random uniform between 0 and 1

# Turn predictions into binary response
(ypred <- as.numeric(pred > 0.5))
...
In this case, we got half of the predictions wrong, i.e. a misclassification
error of 0.5, which is unsurprising since we just used a uniform distribution to
generate the predictions.

These metrics will allow us to compare the performance of different types of
...
To achieve this final model, we need to find the optimal hyperparameters by using
*k-folds cross validation* (CV$_k$) on the training data.
...
a chosen out-of-sample error.  As our training dataset contains now only
80 rows, we can use a maximum of 79 PCs in our model.

The training set is used to fit the 
We use the function `predict()` to calculate
...
for (i in 1:max.n.comps) {
  # Train the model: fit PC regression with i components
...
first one is the raw out-of-sample error, the second one is one corrected
...
#### 4. For all the above prediction models, select the best model in terms of their estimated AUC (out-of-sample error) on the training dataset. Confirm your choice of model on the test dataset. Look at the function `auc` of the package `pROC`. {-}
