...
Read in the data.
The data consists of a subset of the original data where only the 25% most variable genes 
(i.e. 13669 genes) were retained.
...
# download data
data_url <- "https://github.com/statOmics/HDA2020/raw/data/GSE21374-kidney-data.csv.gz"
download.file(data_url, destfile = file.path(out_path, "GSE21374-kidney-data.csv.gz"))
```
...
X <- as.matrix(select(kidney_data, -Patient_ID, -Reject_Status))
...
Note that the gene expression data has already been __centered and scaled__.

```{r}
all(colMeans(X) < 1e-14)
all(abs(matrixStats::colVars(X) - 1) < 1e-14)
```
...
There are several packages available that have a function to compute the AUC.
For example the *[pROC](https://CRAN.R-project.org/package=pROC)* package has the function `auc`,
which can be used with 2 vectors `observations` and `predictions` as `auc(observations, predictions)`.
...
<span style="color:red">__The deadline for submission is ??????.__</span>
