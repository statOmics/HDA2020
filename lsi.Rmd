...
The adjusted $p$-value (also known as the $q$-value in FDR literature):
 \[
   q_{(i)}=\tilde{p}_{(i)} = \min\left[\min_{j=i,\ldots, m}\left(m p_{(j)}/j,1\right) \right].
 \]
...
1. maximum likelihood to estimate the null by only considering the middle part in the distribution of the test statistics (MLE) or
2. by a geometric method that places the best fitting normal under the peak of the estimate of f(z). (CME)
...
Note, that the local fdr method allows us to detect differential brain activity in a specific region of the brain for which a larger activity is observed on average for childeren that do not have dyslexia.
